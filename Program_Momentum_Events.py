   ...
   # taregeted_profit = 0
    ...
    for i in range(0,len(df)):
      #  print(abs(np.log(df['Close'].iloc[i]/df['Close'].iloc[i-4])))
        ...
        if np.log(df['Close'].iloc[i] / df['Close'].iloc[i-long_time]) > (3/3) * targeted_profit_long \
            or np.log(df['Close'].iloc[i] / df['Close'][i-long_time]) < (-1) * (3/3) * targeted_profit_long \
    ...
    for i in range(0,len(df)):
        ...
        if (np.log(df['Close'].iloc[i] / df['Close'].iloc[i-short_time]) < (3/3) * targeted_profit_short \
            or np.log(df['Close'].iloc[i] / df['Close'][i-short_time]) > (-1) * (3/3) * targeted_profit_short) \
